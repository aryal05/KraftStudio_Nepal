...
import { getProductBySlug, getProductById, getProductsByCategory, type ProductData } from "@/data/products";
...
export default function ProductDetail() {
  ...
  // Load product data on mount or when slug changes
  useEffect(() => {
    setIsLoading(true);
    
    // Try to get product by slug first
    let foundProduct = getProductBySlug(productSlug);
    
    // If not found by slug, try by ID (in case old links still use ID)
    if (!foundProduct) {
      const id = parseInt(productSlug);
      if (!isNaN(id)) {
    ...
    if (foundProduct) {
      const related = getProductsByCategory(foundProduct.category, 5)
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mb-6 pb-6 border-b border-gray-200">
                    ...
                    <span className="text-gray-600 font-medium">
                      {rating.toFixed(1)} ({finalProduct.reviewCount || 0} reviews)
                    </span>
                  </div>
                  ...
                  <div className="mb-8">
                    {isInStock ? (
                      <p className="flex items-center gap-2 text-green-600 font-semibold">
                        ...
                        In Stock ({finalProduct.stockQuantity} available)
                      </p>
                    ) : (
                  ...
                  <p className="text-gray-700 text-lg mb-8 leading-relaxed">
                    {finalProduct.description || finalProduct.shortDescription || "No description available."}
                  </p>
                  ...
                  <div className="flex items-center gap-6 mb-8">
                    ...
                    <div className="flex items-center border-2 border-gray-300 rounded-xl overflow-hidden">
                      ...
                      <motion.button
                        ...
                        onClick={() =>
                          setQuantity(
                            Math.min(finalProduct.stockQuantity, quantity + 1)
                          )
                        }
                        className="px-6 py-3 hover:bg-gray-100 transition-colors text-lg font-semibold"
                        disabled={!isInStock}
